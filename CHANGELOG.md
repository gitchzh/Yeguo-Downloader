...
- 🌍 **FFmpeg跨平台集成**
  - 完全移除对ffmpeg.exe的依赖
  - 自动检测系统FFmpeg安装
  - 集成Python原生库（ffmpeg-python, moviepy）
...
- 🏗️ **架构优化**
  - 新增FFmpegManager跨平台管理器
  - 重构file_utils模块，支持多种FFmpeg后端
  - 改进错误处理和用户反馈机制

- 📦 **依赖管理**
  - 添加ffmpeg-python和moviepy依赖
  - 移除硬编码的ffmpeg.exe路径
  - 支持自动安装Python FFmpeg库

- 🌐 **跨平台兼容性**
  - 统一Windows、macOS、Linux的FFmpeg检测逻辑
  - 支持多种包管理器（Homebrew、apt、yum等）
  - 提供平台特定的安装指导
